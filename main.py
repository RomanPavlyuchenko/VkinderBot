...
from VkinderBot import VkinderBot
from VkinderDB import VkinderDB


def get_start_keyboard():
    keyboard = VkKeyboard(one_time=True)

    keyboard.add_button('Изменить параметры поиска', color=VkKeyboardColor.PRIMARY)
    keyboard.add_button('Показать страницы', color=VkKeyboardColor.PRIMARY)
...
def add_new_user(user_id, database: VkinderDB):
    user_info = bot.get_user_info(user_id)
    user_data = {
        'id': user_id,
        'first_name': user_info['first_name'],
        'last_name': user_info['last_name'],
        'gender': user_info['sex'] % 2 + 1
    }
    database.add_user(user_data)


def menu_set_search_params(user_id, db: VkinderDB, bot: VkinderBot):
    search_params = {}

    bot.send_msg(user_id, 'Какого года рождения будем искать?', keyboard=get_empty_keyboard())
    ...
    for event in bot.longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW \
                and event.to_me \
                and event.user_id == user_id:
            if event.text and event.text.isdigit() and int(event.text) in [i for i in range(1, 9)]:
...
def search_users(user_id, db: VkinderDB, bot: VkinderBot):
...
def menu_start(user_id, db, bot):
    if user_is_exist(event.user_id, db):
        cur_user = db.get_user(event.user_id)
    else:
...
def menu_next(user_id, db, bot):
    ...
    if params:
        ...
        if next:
            ...
            bot.send_msg(user_id, message=request['msg'], attachment=request['attach'],
            ...
            if not search_list:
                bot.send_msg(user_id, 'Вариантов нет', keyboard=get_start_keyboard())
                return
...
if __name__ == '__main__':
    ...
    db = VkinderDB('postgresql+psycopg2://vkinder:vkinder@localhost:5432/vkinder')
    # db.drop_all()
    db.init_db()
    menu = {
        'Начать': menu_start,
        ...
        if event.type == VkEventType.MESSAGE_NEW:
            ...
            if event.to_me:
                ...
                if request in menu:
                    ...
                    bot.send_msg(event.user_id, "Не поняла вашего ответа...")
